...
def test_diff_with_no_diff_python():
    ...
    output_code = ['12\n15\n10\n']
    assert list(dwys.diff(input_code=input_code, expected_output_code=output_code, execution_command="python")) == []
...
def test_diff_with_syntax_error_python():
    ...
    with pytest.raises(AssertionError):
        list(dwys.diff(input_code=input_code, expected_output_code=output_code,
        execution_command="python"))
...
def test_diff_with_syntax_error_R():
    ...
    output_code = ['[1] 19\n']
    with pytest.raises(AssertionError):
        list(dwys.diff(input_code=input_code,
        expected_output_code=output_code, execution_command="Rscript")) == []
